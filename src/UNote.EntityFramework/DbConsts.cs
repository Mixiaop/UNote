
namespace UNote.EntityFramework
{
    public class DbConsts
    {
        public class DbTableName
        {
            public const string Notes_Nodes = "UNote_Nodes";
            public const string Notes_Contents = "UNote_Contents";
            public const string Notes_ContentFollowers = "UNote_ContentFollowers";
            public const string Notes_Tags = "UNote_Tags";

            public const string Teams_Teams = "UNote_Teams_Teams";
            public const string Teams_TeamMembers = "UNote_Teams_TeamMembers";

            public const string Users_Users = "UNote_Users";
            public const string Users_UserVisitLogs = "UNote_UserVisitLogs";
        }
    }
}
