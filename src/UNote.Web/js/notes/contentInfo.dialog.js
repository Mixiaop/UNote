define(['jquery', 'utils/notify', 'bootstrap'], function ($, notify) {
    ...
    function _initialize() {
        ...
        $container.find('#btnDownloadPdf').attr('href', "/notes/download.aspx?type=pdf&contentid=" + vm.options.contentId);
        $container.find('#btnDownloadDoc').attr('href', "/notes/download.aspx?type=doc&contentid=" + vm.options.contentId);
    ...
    vm.open = function (options) {
        ...
        if ($container == undefined) {
            $.ajax({
                type: 'get',
                dataType: 'html',
                url: '/js/notes/contentInfo.dialog.html?t=' + new Date().getTime(),
